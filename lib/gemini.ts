...
Return ONLY valid JSON in the following format:
{
  ...
  try {
    ...
    // Validate that it's not partial
    if (text.trim() && (!parsed.corrected_text || parsed.corrected_text.length < (text.length * 0.5))) {
      console.warn("Received potentially truncated response:", parsed.corrected_text);
      if (retryCount < 1) throw new Error("Partial response detected");
    }
